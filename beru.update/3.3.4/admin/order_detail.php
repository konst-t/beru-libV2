...
if (is_array($arMktProductsT)) {
	foreach($arMktProductsT as $prod) {
		$arMktProducts[$prod["id"]] = ["offerID" => $prod["offerId"], "name" => str_replace('"', '\"', TruncateText($prod["offerName"],30))];
		$arMProducts[$prod["offerId"]] = str_replace('"', '\"', TruncateText($prod["offerName"],50));
	}
}
...
if( $PROFILE_ACCESS >= "W"
	...
	if( $request->get("action") == "send_boxes" ) {
		...
		if ($res["status"]==200) {
			...
			$api = new YMAPI($arFields["PROFILE_ID"]);
			$res = $api->getBoxes($arFields["EXT_ID"]);
			$arLabels = [];
			foreach($res["body"]["result"]["parcelBoxLabels"] as $label) {
				$a = explode("/", $label["place"]);
				$arLabels[$label["boxId"]] = $a[0];
			}
			$arBoxes_ = $arBoxes;
			foreach($arSentBoxes as $arSentBox) {
				$boxID = 0;
				foreach($arBoxes_ as $boxKey => $arBox) {
					if(count($arLinks[$arBox["ID"]]) != count($arSentBox["items"])) {
						continue;
					}
					$arFromBase = [];
					$arFromBeru = [];
					$arToBase = [];
					foreach($arSentBox["items"] as $item) {
						if(isset($item["partialCount"])) {
							$arFromBeru[$item["id"]] = [
								"part"  => $item["partialCount"]["current"],
								"parts" => $item["partialCount"]["total"],
							];
						}
						else {
							$arFromBeru[$item["id"]] = [
								"quantity" => $item["fullCount"],
							];
						}
					}
					foreach($arLinks[$arBox["ID"]] as $link) {
						if($link["IS_PART"] == "Y") {
							$arFromBase[$link["ORDER_PROD_ID"]] = [
								"part"  => $link["PART"],
								"parts" => $link["PARTS"],
							];
						}
					...
					if(!($arFromBase == $arFromBeru)) {
						continue;
					}
					$boxID = $arBox["ID"];
					unset($arBoxes_[$boxKey]);
					break;
				}
				 echo $boxID . "<pre>"; print_r(["EXT_ID" => $arSentBox["boxId"], "NUM" => $arLabels[$arSentBox["boxId"]]]); echo "</pre>";
				if ($boxID > 0) {
					$r = BoxTable::update(
						$boxID, 
						["EXT_ID" => $arSentBox["boxId"], "NUM" => $arLabels[$arSentBox["boxId"]]]
					);
				}
			}
